"""
Tests for the real reviewer_agent wiring (core.agents.reviewer_agent):
build_reviewer_agent() and run_reviewer_agent() assembling the two
real, read-only tools (read_research_findings, sandboxed to a
findings-root directory; read_report, sandboxed to a reports-root
...
identical in spirit to tests/agents/test_writer_agent_wiring.py's own
coverage.
"""
...
class _ReadFindingThenCompleteEngine(AgentDecisionEngine):
    """
    Deterministic engine mirroring test_writer_agent_wiring.py's own
    equivalent, but exercising reviewer_agent's read of the SAME
    read_research_findings tool.
    """
...
class _ReadReportThenCompleteEngine(AgentDecisionEngine):
    """
    Deterministic engine exercising reviewer_agent's own read_report
    tool.
    """
...
class _RequestUnavailableShellEngine(AgentDecisionEngine):
    """
    Attempts to invoke a "shell" tool_id that reviewer_agent's real
    wiring never registers -- proving reviewer_agent's tool surface is
...
class _RequestUnavailableWriteReportEngine(AgentDecisionEngine):
    """
    Attempts to invoke writer_agent's own "write_report" tool_id --
    reviewer_agent never registers it (this agent is read-only by
    design), so this must be inert exactly like the shell case.
    """
    ...
    def decide(self, context: AgentContext) -> AgentAction:
        if not context.tool_results:
            return AgentAction(
                ...
                inputs={"filename": "x.md", "content": "x"},
                reason="Attempting to write a report from reviewer_agent.",
            )
...
class _AlwaysOutOfScopePolicyEngine(PolicyEngine):
    ...
    tests/agents/test_writer_agent_wiring.py's own stand-in of the
    same name, proving build_reviewer_agent() genuinely delegates to
    the supplied policy_engine rather than always trusting its own
    registrations.
    """
...
class _AlwaysMisalignedPolicyEngine(PolicyEngine):
    ...
    tests/agents/test_writer_agent_wiring.py's own stand-in of the
    same name, proving build_reviewer_agent() genuinely delegates to
    the supplied policy_engine's config-side alignment check too
    (Build Phase 10), not just its tool-id scope check (Build Phase
    9).
    """
...
def _make_reports_root(*, with_sample=True) -> str:
    root = tempfile.mkdtemp()
    if with_sample:
        # write_bytes(), not write_text(): the fixture content spans
        # multiple lines, and write_text()'s platform-default
        # universal-newline translation would silently write "\r\n"
        # on Windows -- writing raw UTF-8 bytes keeps this fixture
        # (and the exact-content assertions below that read it back
        # through the now-CRLF-fixed read_report executor; see
        # read_report_tool.py's own docstring) platform-independent.
...
# ---------------------------------------------------------------------
# build_reviewer_agent(): tool discovery and configuration errors
# ---------------------------------------------------------------------

def test_build_reviewer_agent_exposes_exactly_the_two_wired_tools():
    ...
    try:
        ...
        assert agent.identity.subject == REVIEWER_AGENT_SUBJECT

        tool_ids = {tool.id for tool in agent.discover_tools()}

        assert tool_ids == {"read_research_findings", "read_report"}
        ...
        assert "write_report" not in tool_ids
...
def test_build_reviewer_agent_raises_clear_error_for_missing_findings_root():
    reports_root = _make_reports_root()
    try:
...
def test_build_reviewer_agent_raises_clear_error_for_missing_reports_root():
    findings_root = _make_findings_root()
    try:
        with pytest.raises(ValueError, match="does not exist"):
            build_reviewer_agent(
...
def test_build_reviewer_agent_raises_when_policy_engine_reports_out_of_scope():
    """
    Genuine delegation proof (Build Phase 9): mirrors
    test_writer_agent_wiring.py's equivalent test. With an otherwise
    completely normal build, injecting a policy_engine whose
    evaluate_agent_scope() reports within_scope=False must still make
    build_reviewer_agent() raise ValueError.
    """
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
    try:
...
def test_build_reviewer_agent_raises_when_policy_engine_reports_misaligned_permissions():
    """
    ...
    reports aligned=False must still make build_reviewer_agent() raise
    ValueError.
    """
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
...
# ---------------------------------------------------------------------
# run_reviewer_agent(): full end-to-end loops
# ---------------------------------------------------------------------

def test_run_reviewer_agent_completes_a_read_research_findings_task():
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
    audit_dir = tempfile.mkdtemp()
    try:
...
def test_run_reviewer_agent_completes_a_read_report_task():
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
    audit_dir = tempfile.mkdtemp()
    try:
        result = run_reviewer_agent(
            "Review report.md.",
            decision_engine=_ReadReportThenCompleteEngine("report.md"),
            findings_root=findings_root,
            reports_root=reports_root,
            audit_log_path=os.path.join(audit_dir, "audit.jsonl"),
            max_steps=3,
...
def test_run_reviewer_agent_requires_llm_client_or_decision_engine():
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
    try:
        with pytest.raises(
...
def test_shell_permission_grants_nothing_because_no_tool_is_registered():
    """
    reviewer_agent never registers a ToolDefinition for resource=shell,
...
def test_reviewer_agent_cannot_write_a_report():
    """
    reviewer_agent is deliberately read-only (see
    core/agents/REVIEWER_AGENT.md's own "Tools > Not allowed" list and
    this module's own docstring): it never registers writer_agent's
    write_report tool, so attempting it must be inert the same way the
    shell attempt above is, not merely denied by permissions.json.
    """
    findings_root = _make_findings_root()
    reports_root = _make_reports_root()
    audit_dir = tempfile.mkdtemp()
    try:
        result = run_reviewer_agent(
            "Try to write a report.",
            decision_engine=_RequestUnavailableWriteReportEngine(),
            findings_root=findings_root,
            reports_root=reports_root,
            audit_log_path=os.path.join(audit_dir, "audit.jsonl"),
            max_steps=3,
